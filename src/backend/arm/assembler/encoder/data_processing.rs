...
mod sxth_props {
    ...
    // SBFM field extractors. SXTH Rd, Rn aliases SBFM Rd, Rn, #0, #15:
    //   sf opc(2) 100110 N immr(6) imms(6) Rn Rd
    fn sf_of(w: u32) -> u32    { (w >> 31) & 1 }      // bit 31
    fn opc_of(w: u32) -> u32   { (w >> 29) & 0x3 }    // bits 30:29
    fn fixed_of(w: u32) -> u32 { (w >> 23) & 0x3F }   // bits 28:23
    ...
    proptest! {
        // ── encode_sxth: SXTH <Rd>, <Rn> aliases SBFM <Rd>, <Rn>, #0, #15 ───────
        //
        // SBFM bit-string: sf opc 100110 N immr imms Rn Rd, with immr=0, imms=15.
        // Independently derived and cross-checked against the reference assembler:
        //   llvm-mc-18 --triple=aarch64 --show-encoding
        //     sxth x0, x0  -> 0x93403c00     sxth w5, w7   -> 0x13003ce5
        //     sxth x9, x2  -> 0x93403c49     sxth xzr,xzr  -> 0x93403fff
        // => 64-bit base 0x93403C00, 32-bit base 0x13003C00, registers OR'd in.
        ...
        fn sxth_reference_encoding(
            ...
            let w64 = expect_word(encode_sxth(&[xreg(rd), xreg(rn)]));
            prop_assert_eq!(w64, 0x93403C00u32 | (rn << 5) | rd);

            let w32 = expect_word(encode_sxth(&[wreg(rd), wreg(rn)]));
            prop_assert_eq!(w32, 0x13003C00u32 | (rn << 5) | rd);
        }
        ...
        //     pinned to the SXTH alias values (#0 / #15).
        #[test]
        fn sxth_field_placement(
            ...
            let dst = if is_64 { xreg(rd) } else { wreg(rd) };
            let w = expect_word(encode_sxth(&[dst, xreg(rn)]));
            prop_assert_eq!(sf_of(w), if is_64 { 1 } else { 0 });
            ...
            prop_assert_eq!(immr_of(w), 0);                  // SXTH: immr = #0
            prop_assert_eq!(imms_of(w), 15);                 // SXTH: imms = #15
            prop_assert_eq!(rn_of(w), rn);
            prop_assert_eq!(rd_of(w), rd);
        }

        // P3. The source register's width is architecturally irrelevant for a
        //     64-bit destination: llvm-mc canonicalizes `sxth x0, x0` to
        //     `sxth x0, w0`, and both assemble to the identical word. The
        //     encoder must therefore produce identical bytes whether the source
        //     is written as Xn or Wn (its number is what enters the Rn field).
        #[test]
        fn sxth_source_width_irrelevant_for_64bit_destination(
            rd in 0u32..=31,
            rn in 0u32..=31,
        ) {
            let via_x = expect_word(encode_sxth(&[xreg(rd), xreg(rn)]));
            let via_w = expect_word(encode_sxth(&[xreg(rd), wreg(rn)]));
            prop_assert_eq!(via_x, via_w);
        }

        // P4. Negative contract: SXTH requires exactly two register operands;
        //     fewer than two must be rejected with Err.
        #[test]
        fn sxth_rejects_too_few_operands(
            n in 0u32..=31,
            missing in 1u32..=2,
        ) {
            let mut ops = vec![xreg(n), xreg(n)];
        ...
        // P5. Negative contract: SXTH takes no immediates. A non-register
        //     operand in either position must be rejected with Err.
        #[test]
        fn sxth_rejects_non_register_operands(
            rd in 0u32..=31,
            rn in 0u32..=31,
            bad_pos in 0u32..2,
        ) {
        ...
        //     reference assembler, SXTH requires source and destination to be
        //     the same width. A 32-bit (W) destination with a 64-bit (X) source
        //     has no valid SBFM encoding (N must equal sf) and is rejected:
        //
        //       $ echo 'sxth w0, x0' | llvm-mc-18 --triple=aarch64
        //       <stdin>:1:10: error: invalid operand for instruction
        //
        //     A conforming encoder MUST return Err. `encode_sxth` instead
        //     ignores the source width and silently emits a 32-bit SBFM word,
        //     accepting an architecturally invalid form. Sibling encoders
        //     sxtb/uxth/uxtb share the same defect.
        #[test]
        fn sxth_rejects_w_destination_with_x_source(
            ...
            prop_assert!(
                ...
                "W destination + X source is architecturally invalid for SXTH; got {:?}",
                encode_sxth(&ops)
            );
        }
    }
