...
`encode_movk` computes the MOVK halfword selector as `amount / 16` for `lsl` shifts. It does not require the amount to be one of the valid MOVK shifts, so non-multiple-of-16 values are silently floored to a different shift.
...
`movk x0, #1, lsl #1` encodes `hw = 1 / 16 = 0`, i.e. no shift, instead of returning `Err`. Likewise `lsl #17` encodes as `lsl #16`. Non-`lsl` shift kinds are also silently treated as `hw = 0`.
