# Bug Report: `encode_neon_mls` silently accepts mismatched arrangement operands
...
`encode_neon_mls` discards the arrangement specifiers of the `Vn` and `Vm`
...
**not** match — e.g. `mls v0.4s, v1.8b, v2.2s` — are silently accepted and
...
This is **distinct** from the already-reported
`encode-neon-mls-accepts-unallocated-doubleword` defect (which is about an
*unallocated* `size=0b11` field); here all three arrangements are individually
*valid*, they are simply *inconsistent*.
...
**Input:** `mls v0.4s, v1.8b, v2.2s`

**Expected:** `Err` — MLS requires all three arrangements to match
(`llvm-mc-18`: `error: invalid operand for instruction`).

**Actual:** `Ok(EncodeResult::Word(0x6EA29420))` — silently coerced to the
destination `.4s` arrangement (identical to `mls v0.4s, v1.4s, v2.4s`).
...
if arr_n != arr_d || arr_m != arr_d {
    return Err(format!(
        "MLS requires matching arrangements; got .{arr_d}/.{arr_n}/.{arr_m}"
    ));
}
let (q, size) = neon_arr_to_q_size(&arr_d)?;
```
...
`mls_rejects_mismatched_arrangement`.
...
(`1`) were verified **correct** for valid inputs via a differential oracle
against `llvm-mc-18`. (Notably, MLS shares opcode `10010` with MLA and differs
