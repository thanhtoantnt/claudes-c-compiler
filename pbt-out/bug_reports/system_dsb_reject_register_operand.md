# Bug: `dsb` accepts a register operand, silently encoding `DSB SY`

**Law:** A `DSB` operand that is not a recognised barrier option name is
invalid and must be rejected with `Err`. A register operand is never a valid
barrier option.

**Impact:** `dsb x0` is silently encoded as `DSB SY` instead of being rejected.
...
successfully and silently inserts a full barrier rather than failing fast.
...
**Minimal input:** `encode_dsb(&[Operand::Reg("x0".into())])`.

**Expected:** `Err(...)` (matches `llvm-mc` *"invalid barrier option name"*).

**Actual:** `Ok(EncodeResult::Word(0xD503_3F9F))` — the catch-all
`_ => 0b1111` arm of `encode_dsb` defaults any non-`Barrier`/`Symbol` operand to
the `sy` option (CRm = 0xF).

**Severity:** medium (invalid operand silently accepted; spurious full barrier).
...
**GitHub Issue:** https://github.com/thanhtoantnt/claudes-c-compiler/issues/329
